...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('client.request-services.update', [$requestService->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('service') ? 'is-invalid' : '' }}" name="service_id"
                        ...
                        @foreach ($services as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('service_id') ? old('service_id') : $requestService->service->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <textarea class="form-control {{ $errors->has('message') ? 'is-invalid' : '' }}" name="message" id="message"
                            required>{{ old('message', $requestService->message) }}</textarea>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <textarea class="form-control {{ $errors->has('place') ? 'is-invalid' : '' }}" name="place" id="place"
                            required>{{ old('place', $requestService->place) }}</textarea>
                        @if ($errors->has('place'))
                            <div class="invalid-feedback">
                                {{ $errors->first('place') }}
                            </div>
